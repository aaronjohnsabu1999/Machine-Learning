...
for i = 1:m
  for j = 1:num_labels
    J = J + (-yMod(i, j)*log(h(i, j)) - (1 - yMod(i, j))*log(1 - h(i, j)));
  endfor
endfor
for i = 1:hidden_layer_size
  for j = 2:(input_layer_size + 1)
    J = J + lambda*(Theta1(i, j)^2)/2;
  endfor
endfor
for i = 1:num_labels
  for j = 2:(hidden_layer_size + 1)
    J = J + lambda*(Theta2(i, j)^2)/2;
  endfor
endfor
J = J/m;
...
for i = 1:m
  ...
  del_3 = H - yMod(i, :);
  del_2 = (Theta2'*del_3')(2:end, :).*sigmoidGradient(Z2);
  
  Del_2 = Del_2 + del_3'*A2;
  Del_1 = Del_1 + del_2*X(i, :);
endfor
